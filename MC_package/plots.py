...
def plot_mass_vs_age(masses,ages,ind):
    ...
    # Extraemos los colores en una lista para usarlos en los gráficos
    ...
    # Resaltar la estrella más vieja y más joven en cada categoría
    for i in range(4):
        cat_ages = ages[ind == i]
        cat_masses = masses[ind == i]
        if len(cat_ages) > 0:  # Asegurarse de que haya datos
    ...
    # Configurar ejes
    plt.xscale('linear')  # Opcional: 'log' si las edades tienen gran rango
    plt.xlabel('Age [Myr]')
...
def pie_plot(ind):
    ...
    # Graficamos el pastel y capturamos los textos de los porcentajes
    wedges, texts, autotexts = plt.pie(fractions, labels=categories, autopct='%1.1f%%', colors=colors)
    
    # Modificar específicamente la posición del porcentaje para "Black Hole"
    for i, category in enumerate(categories):
        if category == 'Black Hole':
            autotexts[i].set_position((1.1, 0))  # Ajusta las coordenadas según necesites
            texts[i].set_position((1.25,0))

        if category == 'Neutron Star':
            autotexts[i].set_position((1.1, -0.08))  # Ajusta las coordenadas según necesites
...
def pie_plot_remanets(ind):
    ...
    fractions = np.array(counts) / sum(counts)  # Recalcula las fracciones sin "Main Sequence"
